...
点击“接口调试”选项卡，或直接访问 `http://127.0.0.1:7788/#debug`（监控入口为 `/#monitor`）。调试页直接调用同源 `POST /rpc`，不是模拟器，不需要另开终端：
...
选项卡切换只改变显示内容，不会中断请求、清空 JSON 输入或调用记录，也不会重复初始化/投放任务。可以投放耗时任务后切换到运行监控观察负载，再返回调试页查看结果；真正刷新整个页面仍会清空页面内存中的记录。选项卡支持方向键、Home/End 和浏览器前进/后退。
